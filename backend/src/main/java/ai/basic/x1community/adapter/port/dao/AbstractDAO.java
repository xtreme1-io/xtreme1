...
public abstract class AbstractDAO<M extends BaseMapper<T>, T> {
    ...
    public boolean saveOrUpdateBatch(Collection<T> entityList, int batchSize) {
        ...
        String keyProperty = tableInfo.getKeyProperty();
        Assert.notEmpty(keyProperty, "error: can not execute. because can not find column for id from entity!");
        return SqlHelper.saveOrUpdateBatch(this.modelClass, this.mapperClass, this.log, entityList, batchSize, (sqlSession, entity) -> {
    ...
    protected boolean executeBatch(Consumer<SqlSession> consumer) {
        return SqlHelper.executeBatch(this.modelClass, this.log, consumer);
    }

    protected <E> boolean executeBatch(Collection<E> list, int batchSize, BiConsumer<SqlSession, E> consumer) {
        return SqlHelper.executeBatch(this.modelClass, this.log, list, batchSize, consumer);
    }
    ...
    public boolean removeById(Serializable id, boolean useFill) {
        ...
        if (useFill && tableInfo.isWithLogicDelete()) {
            if (!modelClass.isAssignableFrom(id.getClass())) {
    ...
    public KtQueryChainWrapper<T> ktQuery() {
        return ChainWrappers.ktQueryChain(this.getBaseMapper(), this.getModelClass());
    }

    public KtUpdateChainWrapper<T> ktUpdate() {
        return ChainWrappers.ktUpdateChain(this.getBaseMapper(), this.getModelClass());
    }
