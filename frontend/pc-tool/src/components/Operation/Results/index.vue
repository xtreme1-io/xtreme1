...
<script setup lang="ts">
    ...
    let filters = computed(() => {
        ...
        sources.forEach((s) => {
            if (s.sourceType === SourceType.TASK) {
                (groundTruth.options as any).push({ value: s.sourceId, label: 'Task ' + s.name });
            } else if (s.sourceType === SourceType.MODEL) {
                (model.options as any).push({ value: s.sourceId, label: s.name });
            }
        });

        if ((groundTruth.options as any).length > 0) filters.push(groundTruth);
        if ((model.options as any).length > 0) filters.push(model);
        return filters;
    });
