import { Rect, Circle, Line, GroupObject, Shape } from '../shape';
import { IRectOption, ToolAction, ToolName, Vector2 } from '../../types';
import ShapeTool from './ShapeTool';
import Konva from 'konva';
import ImageView from '../index';
import * as utils from '../../utils';
import { Cursor, Event, defaultAnchorConfig } from '../../config';
import { handleRotateToCmd } from '../utils/rotate';
...
export default class RectTool extends ShapeTool {
  ...
  anchors: Konva.Group;
  // edit
  object: Rect | undefined = undefined;
  editObjectMap = {} as Record<Anchors | Lines, Shape>;
  transform: Konva.Transformer = new Konva.Transformer({
    resizeEnabled: false,
    ignoreStroke: true,
    // rotationSnaps: [0, 90, 180, 270], // 需要吸附的角度
    // rotationSnapTolerance: 5, // 吸附范围,默认5
    borderEnabled: true,
  ...
  constructor(view: ImageView) {
    ...
    // draw
    this.holder = new Rect({ dash: [5, 5], strokeWidth: 2, visible: false });
  ...
  stopCurrentDraw() {
    ...
    if (this.points.length === 2) {
      const rotation = this.view.editor.state.toolConfig.rectRotate; // 角度
      const width = Math.abs(this.holder.attrs.width);
      const height = Math.abs(this.holder.attrs.height);
      const center = {
      ...
      const pos = utils.countTransformPoint(center, topLeft, rotation);
      rect = new Rect({ ...pos, width, height, rotation });
    }
    this.onDraw(rect);
    this.clearDraw();
  }
  ...
  drawInfo() {
    if (this.holder.visible() || this.object) {
      const { width, height } = this.object?.attrs || this.holder.attrs;
  ...
  onMouseDown(e: Konva.KonvaEventObject<MouseEvent>, point: Vector2) {
    this.addPoint(this.points.length > 0 ? this.currentAnchor.position() : point);
    if (this.points.length >= 2) {
      this.stopCurrentDraw();
    }
  }
  onMouseMove(e: Konva.KonvaEventObject<MouseEvent>, point: Vector2) {
    if (!this.validWithRotation(point)) return;
  ...
  updateHolder() {
    ...
    const deg = this.view.editor.state.toolConfig.rectRotate; // 角度
    const rad = utils.angle2radian(deg); // 弧度
    const dx = endPos.x - startPos.x;
    const dy = endPos.y - startPos.y;
    const v = { x: dx, y: dy };
    const vx = utils.vectorRotate({ x: 1, y: 0 }, rad);
    const vy = utils.vectorRotate({ x: 0, y: 1 }, rad);
    const w = utils.vectorDot(v, vx);
    const h = utils.vectorDot(v, vy);

    this.holder.position(startPos);
    this.holder.width(w);
    this.holder.height(h);
    this.holder.setAttrs({ ...this.drawConfig, rotation: deg });
  ...
  validWithRotation(p: Vector2) {
    // return true;
    const { config, toolConfig } = this.view.editor.state;
    if (!config.limitPosition || toolConfig.rectRotate == 0 || this.points.length == 0) return true;
    ...
    const w = utils.vectorDot(v, vx);
    const h = utils.vectorDot(v, vy);
    const p1 = utils.countTransformPoint(p0, { x: p0.x + w, y: p0.y }, deg);
    const p3 = utils.countTransformPoint(p0, { x: p0.x, y: p0.y + h }, deg);
    return this.isValid(p1) && this.isValid(p3);
  }
  ...
  edit(object: Rect) {
    if (this.object) {
      ...
      this.view.editor.hotkeyManager.unbindRectEvent();
    }
    this._hoverIndex = -1;
    this.object = object;
    ...
    this.addChangEvent();
    this.view.editor.hotkeyManager.bindRectEvent();
  }
  stopEdit() {
    if (this.object) {
      this.removeChangeEvent();
  ...
  initEditObject() {
    ...
    anchors.forEach((id) => {
      const anchor = new Circle({ id, cursor });
  ...
  onAnchorMouseOver(event: KonvaEventObject<any>) {
    const idx = AnchorsOrder.indexOf(event.target.attrs?.id);
    this.hoverIndex(idx);
    this.updateAttrHtml();
  }
  ...
  initEditEvent() {
    ...
    this.editGroup.on(Event.DRAG_MOVE, (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      const newRectInfo = { ...newPos, width: Math.abs(newWidth), height: Math.abs(newHeight) };
      if (this.invalidRect(newRectInfo)) {
        dragNode.position(this.dragLastPos || { x, y });
        return;
      }
    ...
    this.editGroup.on(Event.DRAG_END, (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      endRect = undefined;
      if (this.object) {
        const classCfg = this.view.editor.getClassType(this.object.userData.classId || '');
        this.view.editor.updateObjectFitClass(this.object, classCfg, false);
      }
      this.updateEditObject();
      this.onEditEnd();
      if (this.object) {
    ...
    this.editGroup.on(Event.CLICK, (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      if (e.target instanceof Circle && isAnchors) {
  ...
  updateTransformer() {
    ...
    const anchorStyle = defaultAnchorConfig;
  ...
  updateEditObject() {
    ...
    children.forEach((e) => {
      ...
      if (isAnchors) {
        ...
        const anchorIdx = AnchorsOrder.indexOf(id);
        const p = object.attrs.points[anchorIdx];
        anchor.stroke((p?.attr?.color || object.attrs.stroke) as string);
  ...
  // 检测拖动后的矩形整体是否存在超出限制的部分
  invalidRect(rect: IRectOption) {
    if (!this.view.editor.state.config.limitPosition) return false;
    const { backgroundWidth, backgroundHeight } = this.view;
    const imgRect = { left: 0, right: backgroundWidth, top: 0, bottom: backgroundHeight };
    const r = this.object?.rotation() || 0;

    const points = utils.getRectPointsWithRotation(rect, r);
    const rectSide = utils.getPointsBoundRect(points);
    const { offsetX, offsetY } = utils.transformOffset({ x: 0, y: 0 }, rectSide, imgRect);
    const minNum = Math.pow(2, -50);
    return Math.abs(offsetX) > minNum || Math.abs(offsetY) > minNum;
  }

  invalidSide(val: number) {
    if (!val || Math.abs(val) < 0.0001) return true;
    return false;
  }
  onObjectChange() {
    if (this.dragging) return;
