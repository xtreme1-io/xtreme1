...
import { SelectToolEnum } from '../../types';
...
export function hackContext(view: ImageView, layer: Konva.Layer) {
  ...
  context._stroke = function (shape: Shape) {
    ...
    if (shape.hasStroke()) {
      ...
      if (shape.defaultStyle?.hitStrokeWidth) {
        shape.setAttrs({ hitStrokeWidth: Number(shape.defaultStyle.hitStrokeWidth) * scale });
      }
      ...
      if (hasLinearGradient) {
        ...
        if (shape.userData?.classId === '') {
          shape.setAttr('stroke', view.editor.state.config.defaultResultColor);
        }
  ...
  layer.hitCanvas.context.fillStrokeShape = function fillStrokeShape(shape: Shape) {
    const { selectToolMode } = view.editor.state;
    switch (selectToolMode) {
      case SelectToolEnum.FILL:
        this.fillShape(shape);
        this.strokeShape(shape);
        break;
      case SelectToolEnum.CONTOUR:
        this.strokeShape(shape);
        break;
      default:
        this.fillShape(shape);
