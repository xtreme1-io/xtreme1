<template>
  <div class="tool-item" title="" @click="resetScale()">
    <i class="icon-reset"><IconReset /></i>
    <a-tooltip placement="right" trigger="hover">
      <template #title>
        <div class="rotate-actions">
          <div class="actions-item" @click="onRotate(0)"><IconReset /></div>
          <div class="actions-item" @click="onRotate(-90)"><IconLeftFlip /></div>
          <div class="actions-item" @click="onRotate(90)"><IconRightFlip /></div>
        </div>
      </template>
      <IconScale class="default-icon" />
    </a-tooltip>
    <p>{{ `R:${rotateTxt}` }}</p>
    <p>{{ `Z:${zoomTxt}` }}</p>
  </div>
...
<script setup lang="ts">
  ...
  function onRotate(r: number) {
    if (r !== 0) r += editor.mainView.rotation();
    r = ((r % 360) + 360) % 360;
