<template>
  <teleport to="body">
    <div
      v-if="editor.state.config.showSizeTips && showTips"
      class="help-size"
      :style="{
...
<script lang="ts" setup>
  import { ref } from 'vue';
  import { Event, Shape, ShapeTool, ToolModelEnum } from '../../../image-editor';
  ...
  function updateSizeInfo(drawEdit: string, type: string, object: Shape, tool: ShapeTool) {
    ...
    if (type === 'end' || tool.toolMode === ToolModelEnum.SEGMENTATION) {
